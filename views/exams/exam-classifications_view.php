 		...
 		getGrid: function()
 		{
 			...
 			var Objstore = new Ext.data.Store({
 						proxy: new Ext.data.HttpProxy({
 							url: "<?php echo site_url('examclassifications/getExamClassifications') ?>",
 							method: "POST"
 							}),
    ...
    var colModel = new Ext.grid.ColumnModel([
        ...
        {header: "Description", width: 200, sortable: true, dataIndex: 'description'}
 		    ...
 		    var form = new Ext.form.FormPanel({
 		        labelWidth: 150,
 		        url: "<?php echo site_url('examclassifications/addExamClassification') ?>",
 		        method: 'POST',
 		    ...
 		    _window = new Ext.Window({
 		        ...
 		        buttons: [{
 		            ...
 		            handler: function () {
 			            if(ExtCommon.util.validateFormFields(olexam_category.app.Form)){//check if all forms are filled up
 		                olexam_category.app.Form.getForm().submit({
 			                url: "<?php echo site_url('examclassifications/updateExamClassification') ?>",
 			                params: {id: id},
 			                method: 'POST',
 			                success: function(f,action){
                 		    	Ext.MessageBox.alert('Status', action.result.data);
 		  	...
 		  	olexam_category.app.Form.getForm().load({
 				url: "<?php echo site_url('examclassifications/loadExamClassification') ?>",
 				method: 'POST',
 				params: {id: id},
 				timeout: 300000,
 				waitMsg:'Loading...',
		...
		Delete: function(){
			...
			Ext.Msg.show({
  			...
  			msg: 'Are you sure you want to delete this record?',
   			...
   			Ext.Ajax.request({
                            url: "<?php echo site_url('examclassifications/deleteExamClassification') ?>",
							params:{ id: id},
							method: "POST",
							timeout:300000000,
